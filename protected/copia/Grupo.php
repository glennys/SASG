...
class Grupo extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('parent_id_grupo, descripcion, activo, tipo', 'required'),
			array('parent_id_grupo, tipo', 'numerical', 'integerOnly'=>true),
			array('descripcion', 'length', 'max'=>255),
			array('fecha_creacion, fecha_actualizacion', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_grupo, parent_id_naturaleza, descripcion, activo, tipo, fecha_creacion, fecha_actualizacion', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'observaciones' => array(self::HAS_MANY, 'Observaciones', 'id_grupo'),
			'naturalezaObservacions' => array(self::HAS_MANY, 'NaturalezaObservacion', 'id_grupo'),
			'parentIdNaturaleza' => array(self::BELONGS_TO, 'Grupo', 'parent_id_grupo'),
			'grupo' => array(self::HAS_MANY, 'Grupo', 'parent_id_grupo'),
		);
	}
